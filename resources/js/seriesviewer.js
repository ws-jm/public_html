/*******************
 Series Viewer
 *******************/
 let parameters, hideEmptyRows = false, allow_weekend = true, response_json;
 ...
 window.onload = function () {
    ...
    $.jqx.theme = 'light';
     ...
     function enterDate(data, freq = 'd') {
         if (data) {
             ...
             if (gridMetadata) {
                 ...
                 bates = gridMetadata.Bates.map(function (v, n) {
                     return {
                        id: v + "-" + n,
                        name: v,
                        type: 'float'
             ...
             if(freq == "d"){
                setTimeout(() => {
                    $("#hideDropdown").jqxDropDownList('disableAt', 1);
                }, 300);
            }
     ...
     function CreateAddHeaderRow() {
        ...
        var btns = $("<div id='pop-btns' style='float:right;margin-top:0px;margin-right: 21px;border: 1px solid #ddd;border-top-right-radius:4px;border-bottom-right-radius:4px;'>")
         ...
         $("#hideDropdown").jqxDropDownList({
             ...
             renderer: function (index, label, DatasourceInfo) {
                ...
                if(parameters.Frequency == "d"){
                    $("#listitem1innerListBoxhideDropdown .chkbox .jqx-checkbox-default").css("opacity", 0.5);
                }
                ...
                return '<img height="17" width="17" src="' + icons[index] + '" style="margin-left:7px;"> <span id="databaseDropdown-lable"> ' + label + '</span>';
             },
            ...
            if(getSession() == undefined || getSession() == ""){
                ...
                if (args) {
                    ...
                    if(freq == "d"){
                        ...
                        $("#hideDropdown").jqxDropDownList('enableAt', 1);
                        $("#listitem1innerListBoxhideDropdown .chkbox .jqx-checkbox-default").css("opacity", 1);
                    }
     ...
     for (var i in bates) {
         ...
         if (name == "Adjusted_Close")
             ...
             if (!["Date", "corrected", "correction_count", "correction_bates"].includes(bates[i].name)){
                ...
                columns.push({
                    id: name,
                    name: name+"<p style='color:red'>"+simulated+"</p>",
                    label: name,
                    field: bates[i].name,
                    filtertype: "float",
                    width: 120,
     ...
     $(function () {
         ...
         let y = [];
         for (c in cols) {
             if (cols[c].label !== "#" && cols[c].label !== "Date")
                 y.push(cols[c].label);
         }
     ...
     function resizeElements() {
         ...
         $("#jqxTabs .jqx-tabs-content-element").css("height", "100%");

        //  alert($('#jqxgrid').height());
         setTimeout(() => {
            ...
            for (var i = 0; i < Object.keys(parameters.Series).length; i++) {
                ...
                if(height > 30){
                    $('#jqxgrid .slick-header-columns').css('height', height);
                    // $('#jqxgrid .slick-sort-indicator').css('margin-top', 18);
                }
            }

            if(height == 70){
                $('#jqxgrid .slick-header-columns').css('height', height);
                // $('#jqxgrid .slick-sort-indicator').css('margin-top', 29);
            }

            if(chainged == false){
                $('#jqxgrid .slick-header-columns').css('height', 30);
                // $('#jqxgrid .slick-sort-indicator').css('margin-top', 9);
            }

             $('#jqxgrid').css('width', '100%').css('height', parseInt($('#jqxTabs').height()) - 70);

             var grid_height = $('#jqxgrid').height();
             $('#jqxgrid .slick-pane-top').css('height', (grid_height-height)).css("top", height);
            ...
            //  var grid1_height = $('#jqxgridCorrectionDetails').css('height').slice(0, -2);
    ...
    $('#mainSplitter').css('min-height', (window.innerHeight - $(".navbar").height() - contentBottomPadding + 10) + 'px');
     ...
     function exportData() {
         ...
         Results.forEach(function (RowItem, RowIndex) {
             RowItem.forEach(function (ColItem, ColIndex) {
                 if (ColItem == "") ColItem = " ";
                 ExcelString += '"' + ColItem.toString() + '",';
             });
             ExcelString += "\r\n";
         });
 
         ExcelString = "data:application/excel," + encodeURIComponent(ExcelString);
 
         var link = document.createElement("a");
         link.href = ExcelString;
         link.download = datasource + "-" + symbol + "-EXPORT-" + date + ".xlsx";
         link.click();
     }
