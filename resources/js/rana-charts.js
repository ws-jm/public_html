...
var spanGaps = false,
    ...
    linessubchart, zoomsubchart, candlesubchart, volumesubchart,range_changing;

function getSize(name, fixheight = null, fixwidth = null) {
    var h = $(name).height();
    return {
        width: (fixwidth !== null) ? fixwidth : $(name).width(),
        height: (fixheight !== null) ? fixheight : h,
    }
}
...
function changeRange(lineschart, panel, xMin, xMax) {
    ...
    setTimeout(() => {
        ...
        $('.' + panel).css({
            left: ((pMin - xMin) * $('.' + panel).parent().width()) / (xMax - xMin) + 85,
            width: (((pMax - pMin) * ($('.' + panel).parent().width())) / (xMax - xMin)) - 5
        });
    }, 1);
}
...
function createChart(rowsData, columns) {
    ...
    for (var i = 0; i < rowsData.length; i++) {
        ...
        for(var j = 1; j < columns.length; j++){
            item[columns[j]['id']] = rowsData[i][columns[j]['id']];
        }
        dataArray.push(item);        
    }

    let data = prepareData(dataArray);
    ...
    window.addEventListener("resize", throttle(() => {
        ...
        if (getParameterByName("tab") == "chart" && $('#top-chart').width() > 10) {
            let old_size = zoomchart.width,
                ...
                panel_width = $(".zoom-panel").parent().width();
            zoomchart.setSize(getSize('#top-chart', 55, $('#top-chart').width() - 105));
            let new_size = zoomchart.width;
            $(".zoom-panel").width(((panel_width / old_size) * new_size) - 5);
            $(".zoom-panel").css("left", 5 + (old_position / old_size) * new_size);
        }
    }, 100));
    ...
    */
    $('.zoom-panel')
        .draggable({
            ...
            drag: function (e, ui) {
                lineschart.batch(() => {
                    var _w = $(this).parent().width() - 90;
                    console.log(ui.position.left);
                    ui.position.left < 86 ? ui.position.left = 86 : '';
                    lineschart.setScale("x", {
                        min: (((ui.position.left - 60) * (xMax - xMin)) / _w) + xMin,
                        max: (((ui.position.left + $(this).width() - 60) * (xMax - xMin)) / _w) + xMin
                    });
                });
    ...
    function contextMenuHandler(event) {
        ...
        switch ($.trim($(args).text())) {
            ...
            case "Hide missing values":
                ...
                lineschart.setScale("x", {min: min, max: max});
                break;
            ...
            case "Zoom Reset":
                ...
                $('.zoom-panel').css({left: 85, width: "calc(100% - 100px)"});
                console.log("check3");
                if (linessubchart !== undefined) {
                    linessubchart.setData(data);
                    zoomsubchart.setData(data);
                    $('.zoom-subpanel').css({left: 85, width: "calc(100% - 100px)"});
                }
    ...
    var contextMenuChart = $("#jqxgridMenuChart").jqxMenu({theme: 'light', width: 180, height: (candleActive) ? 120 : 66, autoOpenPopup: false, mode: 'popup'});
    ...
    if (volumeActive)
        volumechart.root.querySelector('.u-over').oncontextmenu = function (event) {
            var scrollTop = $(window).scrollTop();
            var scrollLeft = $(window).scrollLeft();
            contextMenuChart.jqxMenu('open', parseInt(event.clientX) + 5 + scrollLeft, parseInt(event.clientY) + 5 + scrollTop);
            return false;
...
function createSubChart(rowsData, columns) {
    ...
    if (volumeActive) {
        ...
        // $("#top-subchart").width("width", "calc(100% - 115px)");
    ...
    // zoomchart box width can be changed from here
    var ops4_sub = {
        width: $('#top-subchart').width() - 100,
        left: 85,
        height: 45,
        axes: [
            {show: false},
    ...
    window.addEventListener("resize", throttle(() => {
        if (getParameterByName("tab") == "prices" && $('#top-subchart').width() > 10 && !range_changing) {
            let old_size = $(".zoom-subpanel").width(),
                old_position = $(".zoom-subpanel")[0].offsetLeft-85,
                // total_panel_width = $(".zoom-subpanel").parent().width();
                total_panel_width = $($("#zoom-subchart")[0].children[1]).width();
            // to resize the chart under zoom panel, change width of u-over chart
            zoomsubchart.setSize(getSize('#top-subchart', 45, $('#top-subchart').width() - 105));
            let new_size = $('#top-subchart').width() - 105;
            console.log(
                "\nold zoom chart :", total_panel_width,
                "\nnew zoom chart:", new_size,
                "\nold_size range:", old_size,
                "\nnew_size range:",((old_size /  total_panel_width) * new_size),
                // getSize('#top-subchart', 45, $('#top-subchart').width() - 105),
                "\nold left:", old_position,
                "\nnew left ",((old_position / total_panel_width) * new_size),
            );

            $(".zoom-subpanel").width((old_size /  total_panel_width) * new_size);
            $(".zoom-subpanel").css("left", ((old_position / total_panel_width) * new_size)+85);
        }else{
            range_changing=false;
        }
    }, 100));
    ...
    // setTimeout(() => {
        // runs on load of chart for first time
    ...
    // }, 0);
// change size of range sliders width
    $('.zoom-subpanel').css({'width': 'calc(100% - 105px)', 'left': '85px'});
    $('.zoom-subpanel').draggable({
        ...
        drag: function (e, ui) {
            ...
            linessubchart.batch(() => {
                console.log(xMaxs, xMins, " dragging");
                console.log(ui.position.left + ' uni left position');
                ui.position.left < 85 ? ui.position.left = 85 : '';

                // console.log(ui);

                linessubchart.setScale("x", {
                    min: (((ui.position.left - 85) * (xMaxs - xMins)) / $(this).parent().width()) + xMins,
                    max: ((((ui.position.left - 85) + $(this).width()) * (xMaxs - xMins)) / $(this).parent().width()) + xMins
                });
            ...
            if (candleActive)
                candlesubchart.batch(() => {
                    candlesubchart.setScale("x", {
                        min: 85 - ((ui.position.left * (xMaxs - xMins)) / $(this).parent().width()) + xMins,
                        max: 85 - (((ui.position.left + $(this).width()) * (xMaxs - xMins)) / $(this).parent().width()) + xMins
                    });
                });
        ...
        .resizable({
            ...
            resize: function (e, ui) {
                ...
                range_changing=true;
                if (ui.position.left < 85) {
                    ui.position.left = 85;
                }
                linessubchart.batch(() => {
                    console.log("xMaxs:", xMaxs, "\nxMin:", xMins, ui.position.left, "\n Resize\n"
                        , $(this).parent().width(), " parent\n"
                        , ui.size.width, "ui size width"
                    );

                    var total_parts = xMaxs - xMins;
                    var starting_pixels = ui.position.left - 80;
                    var ending_pixels = starting_pixels + ui.size.width;
                    var total_pixels = $(this).parent().width();
// console.log("$(this).parent().width(): ",$(this).parent().width(),"ui.size.width: ",ui.size.width,$(this).parent());
                    linessubchart.setScale("x", {
                        min: ((starting_pixels * total_parts) / total_pixels) + xMins,
                        max: ((ending_pixels) * total_parts) / total_pixels + xMins
                    });
                    ...
                    //     "xMins",xMins,
                    //     "ui.size.width:",ui.size.width,
                    //     "ui.position.left-85:",ui.position.left-80,
                ...
                if (candleActive)
                    candlesubchart.batch(() => {
                        var total_parts = xMaxs - xMins;
                        var starting_pixels = ui.position.left - 85;
                        var ending_pixels = starting_pixels + ui.size.width;
                        var total_pixels = $(this).parent().width();
                        console.log("candleActive");
                        candlesubchart.setScale("x", {
                            // min: (((85 + ui.position.left) * (xMaxs - xMins)) / $(this).parent().width()) + xMins,
                            // max: ((((85 - ui.position.left) + ui.size.width) * (xMaxs - xMins)) / $(this).parent().width()) + xMins
                            min: ((starting_pixels * total_parts) / total_pixels) + xMins,
                            max: (ending_pixels * total_parts) / total_pixels + xMins
                        });
                    });
    ...
    /*============== Chart Menu =============*/
    function subContextHandler(event) {
        var args = event.args;
        ...
        switch ($.trim($(args).text())) {
            case "Candlestick":
                ...
                break;
            case "Hide missing values":
                var isChecked = $("#hideValue").jqxCheckBox("checked");
                ...
                linessubchart.setData(data);
                linessubchart.setScale("x", {min: min, max: max});
                break;
            case "Hints":
                var isChecked = $("#hintSet").jqxCheckBox("checked");
                // $('.chart-hints').toggle(isChecked);
                // if (candleActive) {
                    // candlesubchart.root.querySelector(".u-legend").style.visibility = isChecked ? "visible" : "hidden";
                    $("#legend-subchart .u-legend").css("visibility", (isChecked ? "visible" : "hidden"));
            ...
            case "Zoom Reset":
                linessubchart.setData(data);
                zoomsubchart.setData(data);
                $('.zoom-subpanel').css({left: 85, width: "calc(100% - 115px)"});
                if (lineschart !== undefined) {
                    lineschart.setData(data);
                    zoomchart.setData(data);
                    $('.zoom-panel').css({left: 85, width: "calc(100% - 115px)"});
                }
                if (candleActive) {
                    candlesubchart.setData(candleData);
                    if (candlechart !== undefined)
    ...
    var contextMenuSubChart = $("#jqxgridMenuSubChart").jqxMenu({theme: 'light', width: 185, height: (candleActive) ? 120 : 88, autoOpenPopup: false, mode: 'popup'});
    $("#hideValue").jqxCheckBox({ width: "100%", height: "100%" });
    $("#hintSet").jqxCheckBox({ width: "100%", height: "100%", checked: true });
    if (!candleActive) $("#jqxgridMenuSubChart ul li:first-child").remove();
    contextMenuSubChart.on('itemclick', (event) => {
        subContextHandler(event)
    });
    ...
    if (volumeActive)
        volumesubchart.root.querySelector('.u-over').oncontextmenu = function (event) {
            var scrollTop = $(window).scrollTop();
            var scrollLeft = $(window).scrollLeft();
            contextMenuSubChart.jqxMenu('open', parseInt(event.clientX) + 5 + scrollLeft, parseInt(event.clientY) + 5 + scrollTop);
            return false;
...
function updateChart(rowsData, isChartLoaded, isSubChartLoaded, columns) {
    ...
    if (isChartLoaded) {
        ...
        lineschart.setScale("x", {
            min: (((zoomchart.width - ($('.zoom-panel').width() - 90)) * (xMax - xMin)) / zoomchart.width) + xMin,
            max: xMax
        });
