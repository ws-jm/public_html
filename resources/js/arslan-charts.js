...
function createChart(rowsData) {
    ...
    var ops = {
        ...
        hooks: {
            ...
            setSize:[()=>{ initialise_chart_for_click() }]
        },
        axes: [
            {
                space: 100,
    ...
    */
    ...
    contextMenuChart.on('itemclick', function (event) { contextMenuHandler(event); });
...
function createSubChart(rowsData) {
    ...
    window.addEventListener("resize", throttle(() => {
        if (getParameterByName("tab") == "prices" && $('#top-subchart').width() > 10) {
            let old_size = zoomsubchart.width, old_position = $(".zoom-subpanel")[0].offsetLeft,
                panel_width = $(".zoom-subpanel").parent().width();
    ...
    $('.zoom-subpanel').css({'width':'500px','left':'85px'});
    ...
    contextMenuSubChart.on('itemclick',(event)=>{subContextHandler(event)});
